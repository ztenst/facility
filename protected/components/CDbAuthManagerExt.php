...
class CDbAuthManagerExt extends CAuthManagerExt
{
	...
	public function getItemChildren($names)
	{
		...
		foreach($rows as $row)
		{
			if(($data=@unserialize($row['data']))===false)
				$data=null;
			$children[$row['name']]=new CAuthItemExt($this,$row['name'],$row['type'],$row['chinese'],$row['user_id'],$row['description'],$row['bizrule'],$data,$row['created'],$row['updated']);
		}
		return $children;
	}
	...
	{
		return $this->db->createCommand()
			->delete($this->assignmentTable, 'item_name=:item_name AND user_id=:user_id', array(
				':item_name'=>$itemName,
				':user_id'=>$userId
			)) > 0;
	}
	...
	public function isAssigned($itemName,$userId)
	{
		return $this->db->createCommand()
			...
			->where('item_name=:itemname AND user_id=:userid', array(
				':item_name'=>$itemName,
				':user_id'=>$userId))
			->queryScalar() !== false;
	}
	...
	public function getAuthAssignments($userId)
	{
		...
		foreach($rows as $row)
		{
			if(($data=@unserialize($row['data']))===false)
				$data=null;
			$assignments[$row['item_name']]=new CAuthAssignment($this,$row['item_name'],$row['user_id'],$row['bizrule'],$data);
		}
		return $assignments;
	}
	...
	public function saveAuthAssignment($assignment)
	{
		$this->db->createCommand()
			...
			), 'item_name=:itemname AND user_id=:userid', array(
	...
	public function getAuthItems($type=null,$userId=null)
	{
		...
		{
			$command=$this->db->createCommand()
				->select('name,chinese,t1.user_id,created,updated,type,description,t1.bizrule,t1.data')
				->from(array(
					$this->itemTable.' t1',
					$this->assignmentTable.' t2'
				))
				->where('t1.name=t2.item_name AND t2.user_id=:user_id', array(':user_id'=>$userId));
		}
		else
		{
			$command=$this->db->createCommand()
				...
				->where('t1.name=t2.item_name AND t1.type=:type AND t2.user_id=:user_id', array(
					':type'=>$type,
					':user_id'=>$userId
				));
		}
		...
		foreach($command->queryAll() as $row)
		{
			if(($data=@unserialize($row['data']))===false)
				$data=null;
			$items[$row['name']]=new CAuthItemExt($this,$row['name'],$row['type'],$row['chinese'],$row['user_id'],$row['description'],$row['bizrule'],$data,$row['created'],$row['updated']);
		}
		return $items;
	}
	 ...
	 */
	public function createAuthItem($name,$chinese,$type,$userid,$description='',$bizRule=null,$data=null)
	{
		$this->db->createCommand()
		...
		return new CAuthItemExt($this,$name,$type,$chinese,$userid,$description,$bizRule,$data,time());
	}
	...
	public function getAuthItem($name)
	{
		...
		if($row!==false)
		{
			if(($data=@unserialize($row['data']))===false)
				$data=null;
			return new CAuthItemExt($this,$row['name'],$row['type'],$row['chinese'],$row['user_id'],$row['description'],$row['bizrule'],$data,$row['created'],$row['updated']);
		}
		else
			return null;
	}
